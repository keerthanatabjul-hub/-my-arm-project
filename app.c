...
{
    /*
       Load bulb symbol into CGRAM location 0.
       After this, charlcd(0) will print bulb symbol.
    */
    buildcgram(bulb_symbol, 8);
}
/* =========================================================
    ...
    while(1)
    {
				 ...
				 else if(choice == '4')
        {
            ...
            return;
				...
				{
            lcd_message("EXIT SELECTED", "RESUMING...", 1000);
            return;
        }
        else
        {
...
void app_keypad_task(void)
{
    ...
    */
    ...
    if(key == '+')
    {
        ...
        lcd_message("MODE SELECTED", "AUTO", 800);
    }
    else if(key == '-')
    {
        config_set_mode(MODE_MANUAL_ON);
        lcd_message("MODE SELECTED", "MANUAL ON", 800);
    }
    else if(key == '/')
    {
        config_set_mode(MODE_MANUAL_OFF);
        lcd_message("MODE SELECTED", "MANUAL OFF", 800);
    }
}
...
void app_status_screen(void)
{
    ...
    lcd_gotoxy(0, 0);
    strlcd(config_get_mode_text());
    strlcd(" B:");
    u32lcd(config_get_brightness());

    lcd_gotoxy(1, 0);
    strlcd("ADC:");
    u32lcd(config_get_last_adc());
